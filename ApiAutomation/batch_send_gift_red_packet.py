...
from common.api_paths import RECEIVE_RED_PACKET_PATH, SEND_GIFT_RED_PACKET_PATH
...
def execute_send_gift_only(credential, gift_id, gift_count, total_amount, total_count, condition, distribute_type, delay, verbose=False, retry=1, retry_delay=1.0, jitter=0.3, room_id=None):
    ...
    for attempt in range(1, retry + 1):
        ...
        if is_success(response):
            if verbose:
                logger.info("[OK] %s (ID: %s) - 礼物红包发送成功", phone_number, stay_user_id)
            return {
                "phone": phone_number,
...
def execute_send_gift_and_receive(credential, gift_id, gift_count, total_amount, total_count, condition, distribute_type, delay, verbose=False, retry=1, retry_delay=1.0, jitter=0.3, room_id=None):
    ...
    for attempt in range(1, retry + 1):
        ...
        if verbose:
            logger.info("[SEND_OK] %s (ID: %s) - 礼物红包发送成功, stayRedPacketId=%s", phone_number, stay_user_id, red_packet_id)
        ...
        if is_success(receive_response):
            if verbose:
                logger.info("[OK] %s (ID: %s) - 发礼物红包并抢红包成功, red_packet_id=%s", phone_number, stay_user_id, red_packet_id)
            return {
                "phone": phone_number,
                "stayUserId": stay_user_id,
                "ok": True,
...
def main():
    ...
    if args.skip_receive:
        logger.info("开始批量发送礼物红包（仅发送，跳过抢红包）: total=%s, workers=%s, delay=%s", total, args.workers, args.delay)
        task_func = execute_send_gift_only
    else:
        logger.info("开始批量发送礼物红包并自动抢红包（串联模式）: total=%s, workers=%s, delay=%s", total, args.workers, args.delay)
    ...
    logger.info("批量发送礼物红包完成: 成功=%s/%s, 失败=%s, 耗时=%.1fs", success_count, total, len(failures), elapsed)
